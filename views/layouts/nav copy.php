    ...
    <div class="navbar-fixed">
        ...
        <div class="navbar-center system-type-toggle">
            <button class="toggle-btn <?= $currentSystemType === 'hospital' ? 'active' : '' ?>" data-type="hospital"
                ...
                <a style="text-decoration: none; color: <?= $currentSystemType === 'hospital' ? 'white' : 'black' ?>;"
                    href="index.php?r=hospital/dashboard" data-discover="true">
                    🏥 Hospital
                </a>
            </button>
            <button class="toggle-btn <?= $currentSystemType === 'pharmacy' ? 'active' : '' ?>" data-type="pharmacy"
                onclick="updateSystemType('pharmacy')">
                <a style="text-decoration: none; color: <?= $currentSystemType === 'pharmacy' ? 'white' : 'black' ?>;"
                    href="index.php?r=pharmacy/dashboard" data-discover="true">
                    💊 Pharmacy
                </a>
            </button>
...
<script>
    function doit() {
        localStorage.removeItem('usersSidebarActiveKey')
    }

    // Function to show modules modal
    function showModulesModal(systemType) {
        // Get modules based on system type
        $.ajax({
            url: 'index.php?r=users/getmodules',
            method: 'POST',
            data: {
            ...
            success: function(resp) {
                if (resp.success) {
                    displayModulesModal(systemType, resp.modules, systemType);
                } else {
                    if (typeof showGlobalAlert === 'function') {
                        showGlobalAlert('Failed to load modules.', 'error');
                    }
                }
            },
            error: function() {
    ...
    // Function to display modules in modal (simplified to match system-type-toggle style)
    function displayModulesModal(systemType, modules, systemType) {
        const modal = document.getElementById('globalModal');
        const modalMessage = document.getElementById('globalModalMessage');
        const modalActions = document.getElementById('globalModalActions');
        const modalAlertActions = document.getElementById('globalModalAlertActions');

        // Create modules HTML matching system-type-toggle style exactly
        let modulesHTML = `
        <div class="system-type-toggle" style="justify-content: center; margin: 0;">
    `;
        ...
        if (modules && modules.length > 0) {
            modules.forEach(module => {
                ...
                <button class="toggle-btn" 
                        data-type="${module.link}"
                        onclick="selectModule('${module.link}', '${module.name}', '${systemType}')">
                    ${module.name}
                </button>
            `;
            });
            ...
            <div style="text-align: center; color: #666; padding: 20px;">
                <div>No modules available for ${systemType}</div>
            </div>
        `;
        }
    ...
    `;
        ...
        modalMessage.innerHTML = modulesHTML;
        modalActions.style.display = 'none';
        modalAlertActions.style.display = 'none';

        // Show modal at top center
        modal.classList.add('active');

        // Add outside click to close modal
    ...
    // Function to select a module and update navbar
    function selectModule(moduleLink, moduleName, systemType) {
    ...
    function updateSystemType(type) {
        // Show modules modal instead of directly updating
        showModulesModal(type);
    }
