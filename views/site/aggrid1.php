    ...
    <script>
        ...
        if (!$('style#shimmer-style').length) {
            $('head').append(`
                ...
                /* Cool AG Grid Styling - No Shadows */
                .ag-theme-alpine {
                ...
                .ag-theme-alpine .ag-filter-apply-panel button.ag-filter-apply-panel-button-clear {
                    background: #e5e7eb;
                    color: #374151;
                }
                
                .ag-theme-alpine .ag-filter-apply-panel button.ag-filter-apply-panel-button-clear:hover {
                    background: #d1d5db;
                }
                
                /* Number Filter Styling */
                .ag-theme-alpine .ag-filter-number-to,
                .ag-theme-alpine .ag-filter-number-from {
                    border: 2px solid #e1e5e9;
                    border-radius: 6px;
                    padding: 8px 10px;
                    font-size: 13px;
                ...
                .ag-theme-alpine .ag-filter-number-to:focus,
                .ag-theme-alpine .ag-filter-number-from:focus {
                    border-color: #667eea;
                    outline: none;
                    box-shadow: 0 0 0 3px rgba(102, 126, 234, 0.1);
                }
                ...
                /* Date Filter Styling */
                .ag-theme-alpine .ag-filter-date-from,
                .ag-theme-alpine .ag-filter-date-to {
                    border: 2px solid #e1e5e9;
                    border-radius: 6px;
                    padding: 8px 10px;
                    font-size: 13px;
                ...
                .ag-theme-alpine .ag-filter-date-from:focus,
                .ag-theme-alpine .ag-filter-date-to:focus {
                    border-color: #667eea;
                    outline: none;
                    box-shadow: 0 0 0 3px rgba(102, 126, 234, 0.1);
                }
                ...
                .ag-theme-alpine .ag-set-filter-item-checkbox {
                    margin-right: 8px;
                }
        ...
        $(document).ready(function() {
            ...
            function updateSelectedSizesSummary(selectedSizes) {
                ...
                if (selectedSizes.length > 0) {
                    summary.html('Selected Sizes: ' + selectedSizes.map(s =>
                ...
                fetch('post_requests/post_apis.php', {
                    ...
                    .then(data => {
                        ...
                        if (data.status === 'success' && Array.isArray(data.data) && data.data.length > 0) {
                            // Update AG Grid with new data using new API
                            if (gridApi) {
            ...
            $('.toggle-switch button').on('click', function() {
                ...
                let btns = [];
                $('.toggle-switch button.active').each(function() {
                ...
                const locationName = localStorage.getItem('active_location');
                let selectedSizes = [];
                $('#plotDropdown .dropdown-menu .dropdown-item.active').each(function() {
                    selectedSizes.push({
                        id: $(this).data('size-id'),
                        name: $(this).data('size-name')
                    });
                });
                fetchMemberships(projectId, locationName, selectedSizes, btns);
                filterGridByCategory();
            });
