...
Payment of Rs:' . number_format($_REQUEST['Amount'], 0) . '/- is received against registration no. ' . $mem_result['plotno'] . ' on ' . $model->create_date . '. Thank you';
    ...
    <div>
        ...
        <form method="post" id="main_form" class="main_form" enctype="multipart/form-data" action="#">
    ...
    function get_due_payments(id) {
        ...
        if (id > 0) {
            $.ajax({
                type: "POST",
                url: "index.php?r=memberportal/get_due_payments&id=" + id + "&membership_id=" + membership_id,
                cache: false, // Expecting a JSON response
                ...
                success: function(response) {
                    ...
                    // $('#pay_against').append('<option value="">Select Payment Type</option>');

                    // Loop through response and create new options
                    $.each(response, function(index, item) {
                        // Create a new option element
                        var newOption = $('<option></option>')
                            .val(item.value + ' - ' + item.lab) // Set the value (e.g., "10000")
                            .text(item.lab); // Set the text (e.g., "Booking Charges")

                        // Append the new option to the select
                        $('#pay_against').append(newOption);
                    });

                    // Update the Chosen select to reflect the new options
    ...
    function get_overall_receivables(id) {
        if (id > 0) {
            $.ajax({
                type: "POST",
                url: "index.php?r=memberportal/get_overall_receivables&id=" + id,
                cache: false,
                success: function(response) {
    ...
    function get_overdue(id) {
        if (id > 0) {
            $.ajax({
                type: "POST",
                url: "index.php?r=memberportal/get_overdue&id=" + id,
                cache: false,
                success: function(response) {
                    var amount = parseFloat(response);
                    $('#amount').val(amount);
